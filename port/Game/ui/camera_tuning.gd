...
func present(settings: Dictionary) -> void:
	...
	var dof_allowed: bool = MobileProfile.dof_allowed(settings.quality)
	_dof_toggle.disabled = not dof_allowed
	_dof_toggle.text = "景深模糊" if dof_allowed else "景深模糊（当前画质停用）"
	_sync_dof()
	_sync()
	show()
