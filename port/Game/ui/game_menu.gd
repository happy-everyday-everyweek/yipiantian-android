...
func _ready() -> void:
	...
	for title: String in ["原生（最清晰）", "720p（省电）", "1080p", "1440p", "2160p（4K）"]:
...
func _refresh_dof() -> void:
	if _values.is_empty():
		return
	var allowed: bool = MobileProfile.dof_allowed(_values.quality)
	_dof.disabled = not allowed
	_dof.set_pressed_no_signal(_values.dof_enabled)
	_dof.text = ("已开启" if _values.dof_enabled else "已关闭") + ("" if allowed else " · 当前画质暂不启用")
...
func _wallpaper_button(display: VBoxContainer) -> void:
	# 桌面壁纸由 Win32 宿主进程实现，手机端没有对应能力，不提供入口。
