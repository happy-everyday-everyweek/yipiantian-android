...
const MobileProfile = preload("res://platform/mobile_profile.gd")
const TouchInput = preload("res://platform/touch_input.gd")
...
var mobile_profile: MobileProfile
...
func _ready() -> void:
	...
	# 手机端画质策略：关闭 MSAA 改 FXAA、启用 FSR 上采样、屏幕常亮。
	mobile_profile = MobileProfile.new()
	mobile_profile.apply(get_viewport())
	touch_input = TouchInput.new()
	touch_input.gesture_began.connect(_touch_gesture_began)
	touch_input.orbit.connect(_touch_orbit)
	touch_input.pan.connect(_touch_pan)
	touch_input.zoom.connect(_touch_zoom)
	touch_input.long_press.connect(_touch_long_press)
...
func _open_scene_entry(id: String) -> void:
	if id == "rest":
		if desktop_wallpaper.active and desktop_wallpaper.interacting: _end_wallpaper_interaction()
		return
	if not _tools_available(): return
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if decoration_layout != null and decoration_layout.active:
		...
		return
	# 触摸手势：单指旋转、双指缩放与平移、长按取消。点按仍走触摸转鼠标的点击判定。
	if _consume_touch(event):
		get_viewport().set_input_as_handled()
		return
...
func _physics_process(delta: float) -> void:
	...
	if desktop_wallpaper != null:
		...
		if desktop_wallpaper.active and not desktop_wallpaper.interacting:
			...
			if _wallpaper_click.is_finite():
				...
				if entry.begins_with("tool_") or entry == "rest":
	...
	for pick: Dictionary in picks:
		if selected_tool.is_empty() or $Environment/DoorTools.may_hit(camera,pick.position):
			...
			if not selected_tool.is_empty() and not entry.begins_with("tool_") and entry != "rest": entry=""
			if pick.down:
...
func _touch_gesture_began() -> void:
	# 手势开始时收尾上一次点按判定，避免抬起后把拖动当成田块或农具的点击。
	_cancel_input(false)
	if decoration_layout != null:
		decoration_layout.cancel_pointer_gesture()


func _touch_orbit(relative: Vector2) -> void:
	camera.drag(relative, false)


func _touch_pan(relative: Vector2) -> void:
	camera.drag(relative, true)


func _touch_zoom(amount: float) -> void:
	camera.zoom(amount)


func _touch_long_press() -> void:
	# 长按＝桌面上的右键／Esc：先放下工具，再清除选格或返回全景。
	_cancel_input()
	_cancel_or_return()


func _consume_touch(event: InputEvent) -> bool:
	if touch_input == null:
		return false
	if not (event is InputEventScreenTouch or event is InputEventScreenDrag):
		return false
	return touch_input.handle(event)
...
func _setup_settings() -> void:
	...
	if mobile_profile != null and mobile_profile.is_mobile() and settings_values.get("resolution", "") == "native":
		# 手机屏幕像素密度远高于桌面，原生分辨率性价比最低；默认按 1080 渲染再上采样。
		settings_values.resolution = MobileProfile.DEFAULT_RESOLUTION
	if OS.has_feature("editor") and OS.get_cmdline_user_args().has("--dev-preview"):
...
func _wallpaper_interaction_changed(enabled: bool) -> void:
	...
	if enabled:
		...
		_wallpaper_entry = ""
		if entry.begins_with("tool_"): _open_scene_entry(entry)
		_refresh_hud()
	else:
		_wallpaper_entry = ""


func _restore_wallpaper_layers() -> void:
	for canvas: CanvasLayer in _wallpaper_layers:
...
func _end_wallpaper_interaction() -> void:
	if desktop_wallpaper.busy: return
	_wallpaper_leave()
	# Keep the camera, selected tool, and open pages for the next interaction.
	_wallpaper_layers.clear()
	for node: Node in find_children("*", "CanvasLayer", true, false):
		var canvas := node as CanvasLayer
		if canvas.visible:
			_wallpaper_layers.append(canvas)
			canvas.hide()
