...
func _apply_quality() -> void:
	...
	var msaa: int = Viewport.MSAA_4X if high else Viewport.MSAA_2X
	var positional_atlas: int = 4096 if high else (1024 if low else 2048)
	# Keep distant leaf shadows stable; standard spends the atlas on two cascades.
	var directional_atlas: int = 2048 if low else 4096
	if _mobile:
		# 手机屏幕小、采样密度高，桌面一半的阴影分辨率几乎看不出差别，却省下大块带宽。
		# MSAA 由 mobile_profile 关闭，边缘改由 FXAA 处理。
	...
	if world_environment != null:
		world_environment.ssil_enabled = high and not _mobile
		if _mobile:
...
func _dof_cost_allowed() -> bool:
	return MobileProfile.dof_allowed(_quality)
...
func _process(delta: float) -> void:
	...
	var allowed: bool = not inspecting and _dof_enabled and _dof_cost_allowed() and _dof_strength > 0.0
